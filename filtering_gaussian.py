...
# Define the Gaussian filter kernel size and standard deviation
filter_size = 5  # 5x5 Gaussian filter
sigma = 1.0

# Generate the Gaussian filter kernel
kernel = np.fromfunction(lambda x, y: (1/(2*np.pi*sigma**2)) * np.exp(-((x-filter_size//2)**2 + (y-filter_size//2)**2) / (2*sigma**2)), (filter_size, filter_size))
gaussian_filt = kernel / np.sum(kernel)

# Apply padding to the grayscale image
padded_gray_image = pad_image(gray_image, filter_size)

# Apply the Gaussian filter to the padded grayscale image
filtered_image = apply_filter(gray_image, gaussian_filt)

# Display the original image, grayscale image, Gaussian filter, and filtered image
...
# Gaussian Filter
plt.subplot(2, 3, 4)
plt.imshow(gaussian_filt, cmap='gray', interpolation='none')
